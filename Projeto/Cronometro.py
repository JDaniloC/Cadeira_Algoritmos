...
'''
ARQUIVOS: http://agencia.tse.jus.br/estatistica/sead/odsele/consulta_cand/consulta_cand_2014.zip
...
class Controle:
  ...
  def exibeEmOrdem(self, criterio, ordem, arquivo):
    '''
    ...
    print(self, file = arquivo)
  
  def exibePor(self, criterio, padrao, arquivo):
    '''
  ...
  def remove(self, criterio, padrao, arquivo):
    '''
...
if __name__ == '__main__':
  ...
  resultado.write("INICIO\n")
  print("CARREGANDO CANDIDATOS")
  inicio = time.time()
  controle.loadCandidatos('candidatos')
  print('loadCandidatos: {:.0f} min e {:.0f} segs'.format((time.time() - inicio)/60, (time.time() - inicio)%60), file = tempo)
  print("CANDIDATOS CARREGADOS")
  print("CARREGANDO BENS")
  inicio = time.time()
  controle.loadBens('bens')
  print('loadBens: {:.0f} min e {:.0f} segs'.format((time.time() - inicio)/60, (time.time() - inicio)%60), file = tempo)
  print("BENS CARREGADOS")
  print("DEVOLVE POR")
  inicio = time.time()
  resultado.write("\ndevolvePor\n")
  print(controle.devolvePor('partido', 'PT'), file = resultado)
  print('devolvePor: {:.0f} min e {:.0f} segs'.format((time.time() - inicio)/60, (time.time() - inicio)%60), file = tempo)
  print("DEVOLVE POR FINALIZADO")
  print("EXIBE EM ORDEM")
  inicio = time.time()
  resultado.write("\nexibeEmOrdem\n")
  controle.exibeEmOrdem('alfabetica', 'crescente', resultado)
  print('exibeEmOrdem: {:.0f} min e {:.0f} segs'.format((time.time() - inicio)/60, (time.time() - inicio)%60), file = tempo)
  print("EXIBE EM ORDEM FINALIZADO")
  print("EXIBE POR")
  inicio = time.time()
  resultado.write("\nexibePor\n")
  controle.exibePor('UF', 'PE', resultado)
  print('exibePor: {:.0f} min e {:.0f} segs'.format((time.time() - inicio)/60, (time.time() - inicio)%60), file = tempo)
  print("EXIBE POR FINALIZADO")
  print("MEDIA")
  inicio = time.time()
  resultado.write("\nmedia\n")
  print(controle.media('cargo', 'DEPUTADO FEDERAL'), file = resultado)
  print('media: {:.0f} min e {:.0f} segs'.format((time.time() - inicio)/60, (time.time() - inicio)%60), file = tempo)
  print("MEDIA FINALIZADO")
  print("REMOVE")
  inicio = time.time()
  resultado.write("\nremove\n")
  controle.remove('nascimento', 1990, resultado)
  print('remove: {:.0f} min e {:.0f} segs'.format((time.time() - inicio)/60, (time.time() - inicio)%60), file = tempo)
